...
object CSTFCOO {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //    val outputFile = " /Users/cqwcy201101/Desktop/test.md"
    //
    //    def Num_Itr = 1
    //    val Num_nodes = 2

    def Num_Itr = args.apply(2).toInt
    def Num_nodes = args.apply(3).toInt
    def Rank = 2



    val Data = sc.textFile(inputFile)
    val TensorRdd = FileToTensor(Data)




    def tolerance = 0.000000000000000001
  ...
  def UpdateM(M:IndexedRowMatrix,
    ...
    ToIndexRM( IRMtoRdd(M).leftOuterJoin(m1)
      .mapValues(pair => {if (pair._2.nonEmpty) pair._2.get else VtoBDV(Vectors.zeros(Rank))}))
      .multiply(m2)
...
*/
  ...
  */
            ...
            ) =
  {
    ...
    var MA = Indexed_RowMatrix(OrderSize(Indx_i),Rank,sc,Num_node)
    var MB = Indexed_RowMatrix(OrderSize(Indx_j),Rank,sc,Num_node)
    var MC = Indexed_RowMatrix(OrderSize(Indx_j),Rank,sc,Num_node)
    ...
    def Update_NFM(M:IndexedRowMatrix,
                   ...
                   N:Int) =
    {
      val tmpFM = UpdateM(M,ComputeM1(PairRdd,m2,m3,Index,Rank),ComputeM2(m2,m3),Rank)
      Lambda = UpdateLambda(tmpFM,N)

      NormalizeMatrix(tmpFM,Lambda)
    }
