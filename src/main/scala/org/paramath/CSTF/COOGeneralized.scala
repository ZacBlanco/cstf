...
import breeze.linalg.{dim, norm, pinv, product, sum, DenseMatrix => BDM, DenseVector => BDV}
...
object COOGeneralized {
  ...
  def CP_ALS(tensorData: RDD[Vector],
    ...
    var circular: RDD[(Long, (Vector, Queue[BDV[Double]]))] = tensorData
      .map(v => (v(0).toLong, v))
      .join(matrices(0).rows) // join the ith vector
      .map({
      case(ind: Long, (v: Vector, data: BDV[Double])) => {
        val q = new Queue[BDV[Double]]()
        q.enqueue(data)
        (v(1 % dims).toLong, (v, q))
      }
    }).cache()
    ...
    for(i <- 1 until matrices.length - 1) {
      circular = circular.join(matrices(i).rows)
        .map({ case(ind: Long, ((vec: Vector, vQ: Queue[BDV[Double]]), vNew: BDV[Double])) => {
        vQ.enqueue(vNew)
    ...
    loop.breakable{
      ...
      for (i <- 0 until maxIterations)
      {
        ...
        for(j <- 0 until matrices.length) {
          ...
          circular = circular.join(mat.rows).map({
            case (oldInd: Long, ((vec: Vector, q: Queue[BDV[Double]]), vNew: BDV[Double])) => {
              q.dequeue()
              q.enqueue(vNew)
              (vec(j).toLong, (vec, q))
            }
          ...
          val matRows = circular.mapValues({
            case (vec: Vector, q: Queue[BDV[Double]]) => {
              val vNew: BDV[Double] = q.reduce((v1, v2) => v1 :* v2)
              vNew :* vec(dims)
            }
          }).reduceByKey(_ + _)
          // =================================================================
